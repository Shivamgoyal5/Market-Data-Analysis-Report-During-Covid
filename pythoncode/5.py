...
df = pd.read_excel('GR03.xlsx', thousands=',')
df = df.iloc[:-1]
# Select column 2 (assuming it contains labels) and columns 3, 4, 5
x = df.iloc[:, 1].astype(str)  # Convert to string
y = pd.to_numeric(df.iloc[:, 6], errors='coerce')
z = pd.to_numeric(df.iloc[:, 7], errors='coerce')
m = pd.to_numeric(df.iloc[:, 8], errors='coerce')
...
plt.bar(x_indices, y, width=bar_width, label='Receipts (current US$), 2018', color='mediumblue')
plt.bar([index + bar_width * 1.2 for index in x_indices], z, width=bar_width, label='Receipts (current US$), 2019', color='red')  # Adjust the spacing
plt.bar([index + bar_width * 2.4 for index in x_indices], m, width=bar_width, label='Receipts (current US$), 2020', color='grey')  # Adjust the spacing
